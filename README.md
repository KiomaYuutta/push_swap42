...
if invalid characters and provided, an Error message must be displayed informaing something is wrong with the input
No duplicate numbers are valid
...
To check is the sort is valid, the checker provided, or can be downloaded from the evaluation page, can be piped like before
...
Help from pde-alme to check if program was checking the input correctly
AI was used to understand why when using Radix sort, the movents increased with how bigger the numbers were.
The solution was to use the bits of indexes instead of the bits of the numbers.
</pre>